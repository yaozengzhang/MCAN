...
def get_train_test(target_fn):
    ...
    train_lst_reshaped = np.array(train_lst).reshape(-1, 1)  # Reshape the training set
    train_lst_resampled, _ = ros.fit_resample(train_lst_reshaped, train_lst_reshaped)  # Oversample the training set

    train_lst_resampled = train_lst_resampled.flatten()  # Flatten the oversampled training set back to 1D array

    return train_lst_resampled, test_lst


def get_train_val_test(target_fn):
    train_lst = list()
    val_lst = list()
    test_lst = list()
    ...
    with read_file(target_fn, mode="r") as fin:
        for indx, item in enumerate(fin):
            if item.split("\t")[1] in {"train", "training", "20news-bydate-train"}:
                train_lst.append(indx)
            elif item.split("\t")[1] in {"test", "validation"}:
                test_lst.append(indx)
            else:
                val_lst.append(indx)
...
class PrepareData:
    def __init__(self, args):
        ...
        args.nfeat = graph.number_of_nodes()
...
class TextGCNTrainer:
    def __init__(self, args, model, pre_data, weight_decay=0):
        self.args = args
        self.model = model
    ...
    def prepare_data(self):
        ...
        self.train_lst = self.predata.train_lst

        self.val_lst = self.predata.test_lst

        self.test_lst = self.val_lst
    ...
    def train(self):
        for epoch in range(self.max_epoch):
            ...
            l2_reg = th.tensor(0.0, device=self.device)
...
def main(dataset, times):
    ...
    for ind, seed in enumerate(return_seed(times)):
        ...
        if th.cuda.is_available():
            th.cuda.empty_cache()
    PATH = 'F:/0523_twitter_id.pth'
    th.save(GCN(args.nfeat,
                args.nhid,
                args.nclass,
                args.dropout).state_dict(), PATH)
    print("==> seed set:")
    print(seed_lst)
    record.show_str()
