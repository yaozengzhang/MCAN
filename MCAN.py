...
# from ycygl_weibo_zt import args
...
class Attention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, attention_probs_dropout_prob, ctx_dim=None):
        ...
        # print(hidden_size, self.all_head_size)
        # image_dim = 2048
        if ctx_dim is None:
            ctx_dim = hidden_size
    ...
    def transpose_for_scores(self, x):
        if len(x.size()) == 2:
            x = x.unsqueeze(1)  # Add a dummy dimension for attention head
    ...
    def forward(self, hidden_states, context, attention_mask=None):
        ...
        #print('context_layer', context_layer.shape, "new_context_layer_shape", new_context_layer_shape)
...
class selfAttentionLayer(nn.Module):
    ...
    def forward(self, src, attention_mask=None):
        ...
        new_context = self.self_attention(src, src, attention_mask=attention_mask)

        cross_src = new_context
        # cross_src = torch.cat((cross_src, new_context), dim=2)
...
class DECM(nn.Module):
    def __init__(self, text_model, style_model, image_model, ELA_model, args, dropout=0.1, fusion_dim=128):
        super(DECM, self).__init__()
        self.newly_added_config = args
        self.text_model = text_model
        self.style_model = style_model
        self.image_model = image_model
        self.ELA_model = ELA_model
        # Text_DIM = len(Text_DIM)#768
        # style_DIM = len(style_DIM)#200
        # Image_DIM = len(Image_DIM)#1024
        # ELA_DIM = len(ELA_DIM)#1024

        self.OT_L = CrossAttentionLayer(Text_DIM, style_DIM, nhead=args.cross_n_heads,
                                        dropout=args.dropout)

        self.TE_L = CrossAttentionLayer(Image_DIM, ELA_DIM, nhead=args.cross_n_heads,
                                        dropout=args.dropout)

        L_AV_layer = CrossAttentionLayer((Text_DIM + style_DIM), (Image_DIM + ELA_DIM), nhead=args.cross_n_heads,
                                         dropout=args.dropout)

        self.L_AV = CrossAttentionEncoder(L_AV_layer, args.cross_n_layers)

        total_dim = (Text_DIM + style_DIM + Image_DIM + ELA_DIM)
    ...
    def forward(self, text_output, style_output, image_output, ELA_output, attention_mask=None, token_type_ids=None):
        text_output = torch.unsqueeze(text_output, 1)
        style_output = torch.unsqueeze(style_output, 1)
        image_output = torch.unsqueeze(image_output, 1)
        ELA_output = torch.unsqueeze(ELA_output, 1)
        #print('image_output', image_output.shape, 'ELA_output', ELA_output.shape)
        csa = self.OT_L(text_output, style_output)
        ica = self.TE_L(image_output, ELA_output)

        noverbal_text = self.L_AV(csa, ica)

        L_AV_embedding = noverbal_text[:, 0, :]  #

        fusion = L_AV_embedding

        out = self.fc(fusion)

        return out, fusion, csa, ica, text_output, style_output, image_output, ELA_output  # , noverbal_text_first_layer

    def compute_noverbal_text_first_layer(self, csa, ica):
        """Separate function to compute noverbal_text_first_layer."""
        with torch.no_grad():
            return self.L_AV.layers[0](csa, ica)
