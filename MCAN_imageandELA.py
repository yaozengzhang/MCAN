...
class DECM(nn.Module):
    def __init__(self, text_model, style_model, image_model, ELA_model, args, dropout=0.1, fusion_dim=128):
        ...
        self.OT_L = CrossAttentionLayer(Image_DIM, ELA_DIM, nhead=args.cross_n_heads,
                                        dropout=args.dropout)

        L_AV_layer = selfAttentionLayer(Image_DIM + ELA_DIM, nhead=args.cross_n_heads,
                                        dropout=args.dropout)

        self.L_AV = selfAttentionEncoder(L_AV_layer, 2)

        total_dim = (Image_DIM + ELA_DIM)
    ...
    def forward(self, text_output, style_output, image_output, ELA_output, attention_mask=None, token_type_ids=None):
        ...
        csa = self.OT_L(image_output, ELA_output)

        csa = torch.cat((image_output.squeeze(dim=1), ELA_output.squeeze(dim=1)), dim=1)
        print(csa.shape)
        noverbal_text = self.L_AV(csa)
        ...
        return out, fusion, fusion, fusion, fusion, fusion, fusion, fusion, fusion, fusion
